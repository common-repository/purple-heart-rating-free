...
class WPB_Purple_Heart_Rating_Free extends WPB_Plugin {
	...
	public function settings_page() {
		...
		// General metabox
		add_meta_box( 'wpbph_metabox_general', __( 'General', $this->_plugin_textdomain ), array(
			$metabox_class,
			'general',
		), $this->_settings_menu_slug, 'normal', 'core' );

		// Side metaboxes
		add_meta_box( 'wpbph_metabox_about', __( 'About', $this->_plugin_textdomain ), array(
			$metabox_class,
			'about',
		), $this->_settings_menu_slug, 'side', 'default' );

		add_meta_box( 'wpbph_metabox_links', __( 'Helpful links', $this->_plugin_textdomain ), array(
			$metabox_class,
			'links',
		), $this->_settings_menu_slug, 'side', 'default' );
		...
		<div class="wrap" id="wpbph-settings">
			...
			<form action="options.php" method="post" class="wpbph-settings-form">
				...
				?>
				<input type="hidden" value="<?php echo wp_create_nonce( 'gpaisrpro-options-ajax-nonce' ); ?>"
				       name="gpaisrpro-options-ajax-nonce" id="gpaisrpro-options-ajax-nonce"/>
	...
	public function enqueue_scripts_backend( $hook_suffix ) {
		...
		wp_register_script( 'wpbph_backend_js', $this->plugins_url( 'assets/js/rating-backend.js', $this->_plugin_file ), array(
			'jquery',
			'jquery-ui-core',
			'jquery-ui-slider',
		), null );
	...
	public function enqueue_scripts_frontend() {
		...
		wp_register_script( 'wpbph_frontend_js', $this->plugins_url( 'assets/js/rating-frontend.js', $this->_plugin_file ), array(
			'jquery',
			'jquery-ui-core',
			'jquery-ui-slider',
		...
		wp_localize_script( 'wpbph_frontend_js', 'WPBAjaxRating', array(
	...
	public function rating_frontend( $post = null, $usage = 'frontend' ) {
		...
		<aside itemscope="itemscope" itemtype="http://schema.org/CreativeWork">
			...
			<div class="wpbph-frontend" itemprop="aggregateRating" itemscope="itemscope"
				...
				<div class="wpbph-info">
					<div class="wpbph-info-cell">
						<h1 class="wpbph-headline"
						    data-forid="#wpbph_headline"><?php echo $this->get_option( 'headline' ); ?></h1>

						<p class="wpbph-description"
						   data-forid="#wpbph_description"><?php echo $this->get_option( 'description' ); ?></p>
						<button class="wpbph-button-more" data-forid="#wpbph_button_more"
						        data-title="<?php echo $this->get_option( 'more_button_headline' ); ?>"
						        data-content="<?php echo $this->get_option( 'more_button_description' ); ?>"
						        data-placement="right"
						        data-trigger="<?php echo( ( is_admin() ) ? 'click' : 'hover' ); ?>"><?php echo $this->get_option( 'more_button_label' ); ?></button>
					</div>
				</div>
				<!-- /wpbph-info -->

				<div class="wpbph-rating">
					<img class="wpbph-ajax-loader" style="display:none;"
					     src="<?php echo $this->plugins_url( 'assets/img/ajax-loader.gif' ); ?>" border="0"
					     alt="Ajax Loader"/>
						...
						<div class="wpbph-table">
							<div class="wpbph-table-tr" data-post_id="<?php echo $post->ID; ?>">
								<div class="wpbph-table-td wpbph-table-center wpbph-table-big-heart">
									<i title="<?php echo __( '+1', $this->get_textdomain() ); ?>"
									   class="icon icon-heart wpbph-heart-big" data-current-icon="icon-heart"></i>
								</div>
								<div class="wpbph-table-td wpbph-table-values wpbph-value-right-column">
									<span class="wpbph-value"><span
												class="wpbph-value-inner"><?php echo $rating_percent_ok; ?></span>%</span>
									<span class="wpbph-bad-value"><i
												title="<?php echo __( '-1', $this->get_textdomain() ); ?>"
												class="icon icon-heart wpbph-heart-small"></i> <span
												class="wpbph-bad-value-inner"><?php echo $rating_percent_bad; ?></span>%</span>
								</div>
							</div>
							<div style="clear:both;"></div>
							<div class="wpbph-table-tr" data-post_id="<?php echo $post->ID; ?>">
								<div class="wpbph-table-td wpbph-table-center">
									<button class="wpbph-button-ok"
									        title="<?php echo __( '+1', $this->get_textdomain() ); ?>">
										<i class="icon icon-chevron-up"></i></button>
								</div>
								<div class="wpbph-table-td wpbph-button-right-column">
									<button class="wpbph-button-bad"
									        title="<?php echo __( '-1', $this->get_textdomain() ); ?>">
					...
					<div class="wpbph-copyright-info">
						<a href="https://purpleheartratingplugin.com/"
						   target="_blank"><?php echo __( 'WordPress Rating Plugin by WPBuddy', $this->_plugin_textdomain ); ?></a>
					</div>
				</div>
				<!-- /wpbph-rating -->
			</div>
	...
	public function ajax_rate() {
		...
		if ( ! isset( $_REQUEST['post_id'] ) OR ! isset( $_REQUEST['button_pressed'] ) ) {
			...
			die( json_encode( array(
				'error'   => 1,
				'message' => __( 'Sorry, we cannot proceed your rating.', $this->_plugin_textdomain ),
			) ) );
		}
		...
		if ( ! $post_exists ) {
			...
			die( json_encode( array(
				'error'   => 1,
				'message' => __( 'Sorry, the post you are trying to rate does not exist.', $this->_plugin_textdomain ),
			) ) );
		}
		...
		if ( WPB_Purple_Heart_Rating_Free_Db::has_rated( $post_id ) OR $this->is_post_in_cookie( $post_id ) ) {
			...
			die( json_encode( array(
				'error'   => 1,
				'message' => __( 'You have already rated this post. Thank you!', $this->_plugin_textdomain ),
			) ) );
		}
	...
	public function ajax_refresh_post_ratings() {
		...
		if ( ! isset( $_REQUEST['post_id'] ) ) {
			...
			die( json_encode( array(
				'error'   => 1,
				'message' => __( 'Error while refreshing the current ratings.', $this->_plugin_textdomain ),
			) ) );
		}
	...
	public function admin_notices() {
		...
		<div id="phpr_upgrade_notice" class="updated">
			<div>
				<a id="phpr_upgrade_notice_close"
				   href="<?php echo admin_url( 'plugins.php?wpbph_remove_upgrade_notice=1' ); ?>">x</a>
			</div>
			<p>
				<span id="phpr_upgrade_notice_t1"
				      class="phpr_upgrade_notice_t"><?php echo sprintf( __( '&ldquo;Servus %s! You have been using the free version of the Purple Heart Rating Plugin.&rdquo;', $this->get_textdomain() ), $name ); ?></span>
				<span id="phpr_upgrade_notice_t2"
				      class="phpr_upgrade_notice_t"><?php echo __( '&ldquo;Maybe it\'s a good time to upgrade now to the pro-version?&rdquo;', $this->get_textdomain() ); ?></span>
				<span id="phpr_upgrade_notice_t3"
				      class="phpr_upgrade_notice_t"><?php echo __( '&ldquo;Join many others who are benefitting from the full list of features for their online success!&rdquo;', $this->get_textdomain() ); ?></span>
			</p>

			<p>
				<span id="phpr_upgrade_notice_t1b"
				      class="phpr_upgrade_notice_t phpr_upgrade_notice_tb"><?php echo __( 'Dave - Marketing-Buddy', $this->get_textdomain() ); ?></span>
				<span id="phpr_upgrade_notice_t2b"
				      class="phpr_upgrade_notice_t phpr_upgrade_notice_tb"><?php echo __( 'Flow - Code-Buddy', $this->get_textdomain() ); ?></span>
				<span id="phpr_upgrade_notice_t3b"
				      class="phpr_upgrade_notice_t phpr_upgrade_notice_tb"><?php echo __( 'Duke - Design-Buddy', $this->get_textdomain() ); ?></span>
			</p>

			<a id="phpr_upgrade_notice_btn"
			   href="https://purpleheartratingplugin.com/"
			   target="_blank"><?php echo __( 'COMPARE free & pro', $this->get_textdomain() ); ?></a>
		</div>
		<?php
	}
